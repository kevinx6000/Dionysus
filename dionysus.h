...
class Dionysus{
	...
	public:
		...
		void initResource(const vector<Flow>&);	// Initialize resource usage
		void genDepGraph(const vector<Flow>&);	// Generate dependency graph
	...
	private:
		...
		bool canSchedule(int, const vector<Flow>&);	// Check if some operation o can be scheduled now
